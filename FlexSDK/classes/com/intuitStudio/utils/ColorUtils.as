...
/**
    ...
    **/
		...
		public static function isolateChannelR(color:uint):uint
		{
			return color >> 16 & 0xFF;
		}
		
		public static function isolateChannelG(color:uint):uint
		{
			return color >> 8 & 0xFF;
		}

		public static function isolateChannelB(color:uint):uint
		{
			return color & 0xFF;
		}
    
	    public static function singleChannelR(channel:uint):uint
		{
			return channel << 16;
		}

	    public static function singleChannelG(channel:uint):uint
		{
			return channel << 8;
		}

	    public static function singleChannelB(channel:uint):uint
		{
			return channel;
		}
...
/**
    ...
    **/
		...
		//modify color to darker
		public static function darker (col: uint,amount:Number= .5,offset:Number = 0):uint
		{
			var colA:Array = toRGB(col);
			//var r:uint = Math.max(colA[0] - colA[0] * multiplier - offset,0);
			//var g:uint = Math.max(colA[1] - colA[0] * multiplier - offset,0);
			//var b:uint = Math.max(colA[2] - colA[0] * multiplier - offset,0);
			var r:uint = Math.max(colA[0] * amount - offset,0);
			var g:uint = Math.max(colA[1] * amount - offset,0);
			var b:uint = Math.max(colA[2] * amount - offset,0);
			return r <<16 | g <<8 | b;
		}

		public static function random (maskChannel:uint=0xFFFFFF):uint
		{
			var r:uint = Math.random() * 255 << 16 & maskChannel;
			var g:uint = Math.random() * 255 << 8 & maskChannel;
			var b:uint = Math.random() * 255 & maskChannel;

			return r | g | b;
		}

		public static function get3DLightByAngle (angle:Number,normal:Vector3D):Number
		{
			var light:Vector3D = new Vector3D();
			light.x = Math.cos(angle * 10) * 1;
			light.y = Math.sin(angle * 5) * 2;
			light.z = Math.sin(angle) * Math.cos(angle) * 2;
			light.normalize ();
			light.negate ();

			normal.normalize ();

			var scalar:Number = Math.max(0,light.dotProduct(normal));
...
/**
    * RGB figures show (0x000000 0xFFFFFF up from) the
    * R, G, B returns an array divided into a number from 0.0 to 1.0, respectively.
    *
    * @ Param rgb RGB numbers show (0x000000 0xFFFFFF up from)
    * @ Return array indicates the value of each color [R, G, B,A]
    **/
		public static function toRatio (rgb: uint):Array
		{
			var index:Array = toRGB(rgb);
			var r:Number = index[0] / 255;
			var g:Number = index[1] / 255;
			var b:Number = index[2] / 255;
			var a:Number = 1.0;
			return [r, g, b, a];
		}

        //fill alpha channel data for 24-bit color
        public static function make32BitAlpha(color:uint):uint
		{
